...
class ApiService
{
    ...
    public static function articles($param)
    {
        ...
        $join = [
            ['__AUCTION_MY_PROJECT__ m', 'post.id = m.project_id','left'],
        ];
        ...
        if (!empty($categoryIds)) {
            ...
            $field = !empty($param['field']) ? $param['field'] : 'post.*,count(m.project_id) as number,min(category_post.category_id) as category_id';
            array_push($join, ['__PORTAL_CATEGORY_POST__ category_post', 'post.id = category_post.post_id']);
        }

        $articles = $AuctionProjectModel->alias('post')->field($field)
            ->join($join)
            ->where($where)
            ->where($paramWhere)
            ->order($order)
    ...
    public static function article($id)
    {
        ...
        return $AuctionProjectModel->where($where)->find();
    }
